...
export const get_user_list = (dispatch) => {
    dispatch(request())
    axios.get(`http://localhost:4500/user/Adminuserget/`).then((res) => dispatch(get_user_success(res.data))).catch((err) => dispatch(failure()))
}
...
export const get_singleuserdetails = (id) => (dispatch) => {
    dispatch(request())
    axios.get(`http://localhost:4500/user/Adminuserget/${id}`).then((res) => dispatch(get_singleuser_success(res.data[0]))).catch((err) => dispatch(failure()))
}
